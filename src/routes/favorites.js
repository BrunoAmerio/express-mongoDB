...
//DEVUELVE EL SCHEMA DE FAVORITOS
router.get('/:favoriteId', async (req, res) => {
	const { favoriteId } = req.params;
	if (!favoriteId)
		return res.status(400).json({ error: 'The necesary data was not sent' });

	const result = await Favorite.findOne({
		_id: favoriteId
	});
	if (!result)
		return res.status(400).json({ error: 'The id provided was not found' });
...
router.post('/:favoriteId/:recipeId', async (req, res) => {
	...
	if (!recipeId || !favoriteId)
		return res.status(400).json({ error: 'The necessary data was not sent' });
	...
	if (index >= 0)
		return res
			.status(400)
			.json({ error: 'The recipe is already in favorites' });
	...
	try {
		...
		res
			.status(200)
			.json({ success: 'The recipe has been added to favorites ', result });
	} catch (error) {
