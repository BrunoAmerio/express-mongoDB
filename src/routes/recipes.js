...
router.get('/', (req, res) => {
	...
	axios
		.get(
			'https://api.spoonacular.com/recipes/complexSearch?addRecipeInformation=true&number=100&apiKey=71d188a38c254ab7bfe20099604f1ac8'
		)
...
router.post('/:userId', async (req, res) => {
	...
	if ((!title, !diets, !instructions, !readyInMinutes))
	...
	if (req.body.cuisines) req.body.cuisines = req.body.cuisines.split(',');
	...
	try {
		...
		res
			.status(201)
			.json({ success: 'Recipe create successfuly', recipe: newRecipe });
	} catch (error) {
		res.status(400).json({ error: error.message });
	}
});

//ELIMINAR UNA RECETA CREADA POR EL USUARIO
router.delete('/:userId/:recipeId', async (req, res) => {
	...
	console.log(index, user.recipes);
	...
	try {
		...
		res
			.status(200)
			.json({ success: 'Recipe deleted successfuly', result: user.recipes });
	} catch (error) {
		res.status(400).json({ error: error.message });
	}
});
